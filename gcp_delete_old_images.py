...
def delete_old_docker_images_helper(delete_images, base_cmd,
    ...
    print(f"deleting docker images {'with tags' if with_tags else 'without tags'}:\n" +
          '\n'.join(delete_images))

    for image in delete_images:
        # '--delete-tags' flag shouldn't be added when deleting images without
        # tags to catch unseen errors
...
def delete_old_docker_images():
    ...
    for image in images_without_tags:
        if image['version'] in latest_images_manifests:
            print(f"Not deleting {image['package']}@{image['version']} because" +
                  "it is manifest of one of the latest images'")
        else:
